...
class facebook_WT_Module extends WT_Module implements WT_Module_Config, WT_Module_Menu {
    ...
    private function admin() {
        ...
        echo '<style scoped>',
            ...
            '  background-image:url(/themes/_administration/images/admin_icon_sprite.png);',
            '  background-position: -40px -154px;',
            '  background-color: transparent',
            '}',
            '</style>',
            '<h4>'.WT_I18N::translate('Facebook API').'</h4>',
            '<form method="post" action="">',
            '<p>The App ID and secret can be setup at <a href="https://developers.facebook.com/apps">https://developers.facebook.com/apps</a>.</p>',
            '<label>'.WT_I18N::translate('App ID:').' <input type="text" name="app_id" value="', get_module_setting($mod_name, 'app_id', ''), '"></label> ',
            '<label>'.WT_I18N::translate('App Secret:').' <input type="text" name="app_secret" value="', get_module_setting($mod_name, 'app_secret', ''), '" size="40"></label></p>',
                  '<p><label><input type="checkbox" name="require_verified" value="1" ', (get_module_setting($mod_name, 'require_verified', 1) ? 'checked="checked" ' : ''),'>Require verified Facebook accounts <em>(Only disable for testing)</em></label></p>',
            '<p><input type="submit" name="saveAPI" value="', WT_I18N::translate('Save'), '"></p>',
            '</form>',
            '<hr/>',
            '<h4>'.WT_I18N::translate('Linked users').'</h4>',
            '<form method="post" action="">',
            '<p>'.WT_I18N::translate("Associate a webtrees user with a Facebook account.").'</a></p>',
            '<table>',
            '<thead>',
            '<tr>',
            '<th>'.WT_I18N::translate('webtrees Username').'</th>',
            '<th>'.WT_I18N::translate('Real name').'</th>',
            '<th>'.WT_I18N::translate('Facebook Account').'</th>',
            '</tr>',
            '</thead>',
            '<tbody>';
        ...
        echo '<tr>',
            ...
            '<td><input type="submit" name="addLink" value="', WT_I18N::translate('Add'), '"></td>',
            '</tr>',
            '</tbody>',
            '</table>',
            '</form>',
            '<hr/>',
            '<h4>'.WT_I18N::translate('Pre-approve users').'</h4>',
            '<form method="post" action="">',
            '<p>'.WT_I18N::translate("If you know a user's Facebook username but they don't have an account in webtrees, you can pre-approve one so they can login immediately the first time they visit.").'</a></p>',
            '<table>',
            '<thead>',
            '<tr>',
            '<th>'.WT_I18N::translate('Facebook Account').'</th>',
        ...
        echo '<tr>',
            '<td><input type="text" name="facebook_username"/></td>',
            '<td><input type="submit" name="savePreapproved" value="', WT_I18N::translate('Add'), '"></td>',
            '</tr>',
            '</tbody>',
            '</table>',
            '</form>';
